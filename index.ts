...
const startKeyboard = {
  inline_keyboard: [
    [{ text: "🔴 بدء تسجيل", callback_data: "start_rec" }],
    [{ text: "🚀 تجهيز مسبق", callback_data: "prepare" }],
  ],
};
...
async function handleStart(chatId: number) {
  ...
  await tg("sendMessage", {
    chat_id: chatId,
    text: "أهلاً 👋 هذا بوت التسجيل.\n\n🔴 بدء تسجيل — يشتغل على طول (فيه تأخير بسيط أول مرة).\n🚀 تجهيز مسبق — يجهّز الأكشن الآن، وبعدين أي تسجيل تبدأه يشتغل فورًا بدون تأخير.",
    reply_markup: startKeyboard,
  });
}

async function triggerStandbyAction(chatId: number) {
  const res = await fetch(
    `https://api.github.com/repos/${GH_REPO}/actions/workflows/record.yml/dispatches`,
    {
      ...
      body: JSON.stringify({
        ref: "main",
        inputs: { status_chat_id: String(chatId) }, // source_url فاضي = وضع استعداد
      }),
    }
  );
  if (!res.ok) {
...
async function handleCallback(cb: any) {
  ...
  if (data === "prepare") {
    const rows = await sbJson(`standby?id=eq.main&select=*`);
    const row = rows[0];
    if (row && (row.status === "ready" || row.status === "pending")) {
      await tg("sendMessage", { chat_id: chatId, text: "فيه أكشن مجهّز أصلاً — جاهز تستخدمه، اضغط 🔴 بدء تسجيل." });
      return;
    }
    await sb(`standby?id=eq.main`, {
      method: "PATCH",
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify({ status: "pending" }),
    });
    try {
      await triggerStandbyAction(chatId);
      await tg("sendMessage", { chat_id: chatId, text: "🚀 جاري تجهيز الأكشن... بتوصلك رسالة تأكيد خلال دقيقة تقريبًا." });
    } catch (err) {
      await tg("sendMessage", { chat_id: chatId, text: `❌ فشل التجهيز: ${(err as Error).message}` });
    }
    return;
  }
...
async function handleText(chatId: number, text: string, session: any) {
  ...
  // نتحقق هل فيه أكشن مجهّز فعليًا ينتظر (وضع استعداد) — لو نعم، نكلّفه مباشرة
  // (أسرع بكثير، ثوانٍ بس)، بدل ما نفتح مهمة جديدة من الصفر (تأخير معتاد).
  const standbyRows = await sbJson(`standby?id=eq.main&select=*`);
  const standby = standbyRows[0];

  if (standby && standby.status === "ready") {
    await sb(`standby?id=eq.main`, {
      method: "PATCH",
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify({
    ...
    await tg("sendMessage", {
      chat_id: chatId,
      text: `⚡ فيه أكشن مجهّز أصلاً — التسجيل بيبدأ خلال ثوانٍ.`,
